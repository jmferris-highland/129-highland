...
```
/home/nodered/config/
├── device_catalog.json        ← git: yes (manually curated model metadata: battery specs, reporting intervals)
├── device_registry.json       ← git: yes (HA-sourced device/area data — fully generated, no manual entries)
...
> **`device_catalog.json`** is the authoritative home for all manually curated model-level metadata that cannot be derived from HA — battery chemistry, battery quantity, rechargeable flag, and (future) expected reporting intervals for dropout detection. It is never written by any flow.

> **`device_registry.json`** is fully generated by `Utility: Device Registry` on every refresh. It contains no manually curated data. Do not add entries to this file manually — they will be overwritten on next refresh.
...
3. Store in global context:
   - `global.config.device_catalog`
   - `global.config.device_registry`
   - `global.config.flow_registry`
   ...
   > **Note:** `global.config.device_registry` is overwritten shortly after startup by `Utility: Device Registry` with a fresh HA pull. Config Loader's load of `device_registry.json` serves as the initial seed only. `global.config.device_catalog` is loaded from `device_catalog.json` and is never overwritten by any flow.
